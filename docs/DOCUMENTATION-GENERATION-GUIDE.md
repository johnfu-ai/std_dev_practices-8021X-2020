...
This directory contains tools for generating comprehensive API documentation from the wpa_supplicant-8021X-2020 Library source code using Doxygen.
...
cmake -B build -Dwpa_supplicant-8021X-2020_BUILD_DOCS=ON
...
└── wpa_supplicant-8021X-2020.tag     # Tag file for external refs
```
...
- `@ieee` for IEEE 802.1X-2020 section references

**Example**:
```cpp
/**
 * @file clocks.hpp
 * @brief IEEE 802.1X-2020 Clock State Machines
 * @details Implements Ordinary Clock, Boundary Clock, and Transparent Clock
 *          state machines per IEEE 802.1X-2020 Sections 9 and 10.
 * 
 * @note Deterministic design: No dynamic allocation, no blocking calls
 * @ieee IEEE 802.1X-2020, Section 9.2 "protocol port state machines"
 * @req REQ-F-202 Deterministic BMCA
 */

/**
 * @brief Protocol Port State Machine
 * @details Implements port state machine per IEEE 802.1X-2020 Section 9.2
 * 
 * State transitions:
 ...
 * 
 * @ieee IEEE 802.1X-2020, Section 9.2.6 "State decision algorithm"
...
class PtpPort {
     ...
     * @return ProtocolResult<void> Success or error
     * 
     ...
     * @ieee IEEE 802.1X-2020, Section 9.5.2 "Announce message processing"
     */
    ProtocolResult<void> process_announce(
...
```
PROJECT_NAME           = "wpa_supplicant-8021X-2020 Library"
PROJECT_NUMBER         = 1.0.0-MVP
PROJECT_BRIEF          = "Hardware-agnostic protocol implementation"
 ...
 * @return ProtocolError Success or error code
 * 
 * @deterministic Bounded execution time: O(1)
 * @realtime Non-blocking operation
 * @req REQ-NF-007 Clock adjustment accuracy ±1 ppb
 * @ieee IEEE 802.1X-2020, Section 11.6 "Clock adjustment"
 */
ProtocolError adjust_frequency(double ppb_adjustment);
```
...
git remote add origin https://github.com/[org]/wpa_supplicant-8021X-2020.git
...
project = 'wpa_supplicant-8021X-2020 Library'
extensions = ['breathe']
breathe_projects = {"wpa_supplicant-8021X-2020": "../doxygen/xml"}
breathe_default_project = "wpa_supplicant-8021X-2020"
```
...
- IEEE 802.1X-2020 (802.1X-2020 specification)
