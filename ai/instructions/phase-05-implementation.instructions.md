...
## ⚠️ PROJECT-SPECIFIC RULES — IEEE 802.1X-2020 / wpa_supplicant

> These rules override generic template guidance below for this project.

### Code Location — ALL C code goes in `wpa_supplicant-8021X-2020/`

This repo (`std_dev_practices-8021X-2020`) holds **only documentation evidence** for Phase 05:
...
**Never create `.c`, `.h`, or build files in this repo.**

### Where to write C code

| What to implement | Target file in `wpa_supplicant-8021X-2020/` |
|---|---|
| Logon Process state machine (Clause 12) | `src/pae/ieee802_1x_logon.c` + `.h` |
| ANCP (Announced Network Connectivity) | `src/pae/ieee802_1x_ancp.c` + `.h` |
| Extend Supplicant PAE (Clause 8) | `src/eapol_supp/eapol_supp_sm.c` |
| Extend MKA / KaY (Clause 9) | `src/pae/ieee802_1x_kay.c` |
| New wpa_supplicant ↔ feature bridge | `wpa_supplicant/wpas_<feature>.c` |

### Where to write test code

| Test type | Target location |
|---|---|
| Unit tests (mock injection) | `wpa_supplicant-8021X-2020/src/pae/tests/test_ieee802_1x_<feature>.c` |
| EAPOL integration tests | Extend `wpa_supplicant-8021X-2020/wpa_supplicant/eapol_test.c` |

### Build command (wpa_supplicant Makefile — never CMake)

```bash
cd wpa_supplicant-8021X-2020/wpa_supplicant
make -j$(nproc)
# Check errors only (fast feedback):
...
```makefile
# In wpa_supplicant-8021X-2020/wpa_supplicant/Makefile:
...
### C code traceability header (use this, not TypeScript examples below)

```c
/*
 * ieee802_1x_logon.c — Logon Process state machine
 *
 * Implements: #REQ-F-PAE-012 (Logon Process NID selection)
 * Architecture: #ADR-PAE-003 (Logon Process integration point)
 * Verified by: #TEST-PAE-012
...
### C unit test pattern (mock injection via function pointers)

```c
/*
 * test_ieee802_1x_logon.c — Unit tests for Logon Process
 *
 * Verifies: #TEST-PAE-012
 * See: IEEE 802.1X-2020, Clause 12.5
 */
#include "utils/common.h"
#include "ieee802_1x_logon.h"

/* --- Mock context --- */
static int mock_send_eapol(void *ctx, const u8 *dest, u16 proto,
...
static struct ieee802_1x_logon_ctx mock_ctx = {
    .send_eapol = mock_send_eapol,
};

/* --- Test: NID selection triggers CONNECTING --- */
static void test_logon_nid_selection(void)
{
    struct ieee802_1x_logon *sm = ieee802_1x_logon_init(&mock_ctx);
    /* assert sm != NULL */
    /* trigger event, assert state == LOGON_CONNECTING */
    ieee802_1x_logon_deinit(sm);
}

int main(void)
{
    test_logon_nid_selection();
    wpa_printf(MSG_INFO, "All Logon Process tests passed");
    return 0;
}
```
...
```
Red:    Add test_ieee802_1x_<feature>.c, compile → linker error (function missing)
Green:  Add ieee802_1x_<feature>.c with minimum implementation → test passes
Check:  make -j$(nproc) && ./eapol_test ... → integration confirms behaviour
Refactor: Clean up, add Doxygen header, reference standard clause, commit
```
 ...
 * 
 * @see https://github.com/wpa_supplicant/wpa_supplicant-8021X-2020/issues/45
 * @see https://github.com/wpa_supplicant/wpa_supplicant-8021X-2020/issues/78
 */
...
**Prompt**: Use `ai/prompts/reliability-plan-create.prompt.md`
...
**Prompt**: Use `ai/prompts/operational-profile-create.prompt.md`
...
**Prompt**: Use `ai/prompts/sfmea-create.prompt.md`
