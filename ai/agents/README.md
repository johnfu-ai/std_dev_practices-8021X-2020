# AI Agent Profiles for Standards-Compliant Development

This directory contains canonical agent profiles for standards-compliant development across multiple AI tools. The `.github/agents/` directory is a compatibility projection generated from this source-of-truth.

## 📋 Available Agents

These agent profiles compose the focused capabilities documented in `../skills/README.md`.
...
Use `../skills/README.md` to map broad agents onto focused capabilities:

- `RequirementsAnalyst` -> requirements traceability + domain model
- `ArchitectureStrategist` -> architecture governance + domain model
- `TDDDriver` -> wpa_supplicant TDD implementation + verification
- `TestingSpecialist` -> verification and validation + traceability
- `DocumentationExpert` -> documentation governance + architecture support
- `SecurityAnalyst` -> security review + domain model
...
2. Phase-specific instructions in `ai/instructions/phase-NN-*.instructions.md`
3. Specialized agents in `ai/agents/*.md`
