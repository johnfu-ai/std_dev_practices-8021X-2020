# Root AI Instructions - Standards-Compliant Software Development

**Canonical source**: `ai/instructions/root.instructions.md`  
**Projected to**: `.github/copilot-instructions.md`

Do not edit `.github/copilot-instructions.md` directly — run `python3 scripts/sync-ai-adapters.py` after editing this file.
...
## ⚠️ PROJECT-SPECIFIC CONSTRAINTS (Read First)

This methodology repository is the **lifecycle documentation and traceability hub** for the **IEEE 802.1X-2020 compliance project**. It does NOT contain implementation code.

### Implementation Target: wpa_supplicant (Extension Model)
...
- ❌ Generate CMake library targets for 802.1X protocol code
- ❌ Create `lib/Standards/` or `include/` C/C++ files in this repo
- ❌ Suggest adding new build systems (Meson, Bazel, etc.) on top of wpa_supplicant
- ❌ Create C++ namespaces or C++ class wrappers around wpa_supplicant C code

### What Agents MUST Do Instead
- ✅ Document requirements, architecture decisions, tests in the `01-09` phase folders here
...
```
<workspace-root>/
├── wpa_supplicant-8021X-2020/   # C implementation (Makefile, wpa_supplicant stack)
├── 8021X-2020.md/               # IEEE 802.1X-2020 standard reference (read-only)
├── 8021X-2020.YANG/             # Normative YANG data models (reference)
└── std_dev_practices-8021X-2020/ # ← THIS REPO: lifecycle docs only
```
...
**Location**: `05-implementation/` (evidence and tracking docs here; C code in `wpa_supplicant-8021X-2020/`)  
**Standards**: ISO/IEC/IEEE 12207:2017 (Implementation Process)  
**XP Focus**: TDD (Red-Green-Refactor), Empirical Validation, Continuous Integration  
**Build**: `cd wpa_supplicant-8021X-2020/wpa_supplicant && make -j$(nproc)` — wpa_supplicant Makefile only  
**Critical Rule**: Write new code ONLY if an automated test has failed  
**Objective**: Extend wpa_supplicant C source files to implement 802.1X-2020 clauses; prove correctness through eapol_test and function-pointer mock injection; document implementation evidence in `05-implementation/`
...
See: https://github.com/wpa_supplicant/wpa_supplicant-8021X-2020/issues/123
"""
...
4. **Use Phase-Specific Instructions** - Phase-specific guidance is auto-applied based on file location via `ai/instructions/phase-NN-*.instructions.md`
...
- Phase-specific instructions: `ai/instructions/phase-NN-*.instructions.md` (auto-applied by file location)
